...
import org.aklimov.fall_analytics.lib.services.domain.GrowChangeStatRes
import org.aklimov.fall_analytics.lib.services.domain.Ticker
import org.nield.kotlinstatistics.median
import org.nield.kotlinstatistics.percentile
import org.springframework.jdbc.core.namedparam.NamedParameterJdbcTemplate

class Stats(private val ohlcDao: OhlcDao) {

    fun growChangeStat(period: Int, ticker: Ticker): GrowChangeStatRes {
        require(period > 0)

        val lst = ohlcDao.loadPoints(ticker).sortedBy(Point::date)
        ...
        val onlyGrowChngs =
            lst.windowed(period + 1) // We take period+1 for getting OPEN last cndl. before period batch
                .filter(Utils.Companion::checkListOfGrow)
                .map(Utils.Companion::computeChngOfWnd)

        return GrowChangeStatRes(
            ticker = ticker,
            period = period,
    ...
    fun searchGrowMoreThenPercentile(
        stat: GrowChangeStatRes,
        growMoreThenPercentile: ValidPercentilesEnum
    ): GrowMoreThenPercentileResult {

        logger.info { "Search periods [by ${stat.period}] has grow more than ot equals $growMoreThenPercentile percentile" }

        val growsPercentiles = ohlcDao.loadPoints(stat.ticker)
            .sortedBy(Point::date)
